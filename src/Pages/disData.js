...
const DisData = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axiosInstance.get(`/discharge-data/${id}?${queryString}`, {
  ...
  useEffect(() => {
    const fetchMeta = async () => {
      try {
        const response = await axiosInstance.get(`/discharge-station-data/${id}`, {
          headers: { Accept: "application/json" },
        });
        if (response.data !== null) {
          setMeta(response.data);
  ...
  const sortKey = (key, type) => {
    const sortedData = [...(data || [])].sort((a, b) => {
      if (key === "value") return type === 1 ? a[key] - b[key] : b[key] - a[key];
      return type === 1 ? a[key].localeCompare(b[key]) : b[key].localeCompare(a[key]);
    });
    setSort(`${key}${type}`);
    setData(sortedData);
  };
    ...
    <div className="container mx-auto w-full ">
      ...
      <div className="flex w-1/2 justify-between mt-5">
        <div className="ml-16 font-semibold text-xl">
          ...
          <p>Province: {meta.province}</p>
          <p>Status: {meta.status}</p>
          <p>Latitude: {meta.latitude?.toFixed(4) || "N/A"}</p>
          <p>Longitude: {meta.longitude?.toFixed(4) || "N/A"}</p>
        </div>

        <div className="grid mr-16">
          <Link to={`/graph?station=${meta.stationName} - ${meta.stationNo} dis`} className="font-semibold text-xl text-blue-500">
            Plot Graph
          </Link>
          <Link to={`/map?lat=${meta.latitude}&&long=${meta.longitude}&&zoom=15`} className="font-semibold text-xl text-blue-500">
            View on Map
          </Link>
        </div>
      </div>
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            <DatePicker
              ...
              onChange={(newValue) => newValue && setStartDate(newValue.format("YYYY-MM-DD"))}
              slotProps={{ textField: { size: "small" } }}
            />
            <Typography>End Date:</Typography>
            <DatePicker
              value={dayjs(endDate)}
              onChange={(newValue) => newValue && setEndDate(newValue.format("YYYY-MM-DD"))}
              slotProps={{ textField: { size: "small" } }}
            />
          </Stack>
        </LocalizationProvider>
      ...
      {!loading && error === null ? (
        <div className="p-6">
          {/* Your existing table rendering code unchanged */}
        </div>
      ) : error === null ? (
        <p>loading</p>
      ) : (
